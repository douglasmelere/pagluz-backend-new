...
let ConsumersService = class ConsumersService {
    ...
    async getRepresentativeConsumerStats(representativeId) {
        ...
        const totalKwh = consumers.reduce((sum, c) => sum + (Number(c.averageMonthlyConsumption) || 0), 0);
        const allocatedKwh = consumers
            .filter((c) => {
            const status = String(c.status).toUpperCase();
            const isAllocatedLike = status === "ALLOCATED" || status === "CONVERTED";
            const hasAllocation = (Number(c.allocatedPercentage) || 0) > 0 && Boolean(c.generatorId);
            return isAllocatedLike && hasAllocation;
        })
            .reduce((sum, c) => sum +
            ((Number(c.averageMonthlyConsumption) || 0) * (Number(c.allocatedPercentage) || 0)) / 100, 0);
        let totalDiscount = 0;
