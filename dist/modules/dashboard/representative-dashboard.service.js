...
let RepresentativeDashboardService = class RepresentativeDashboardService {
    ...
    calculateRepresentativeStats(consumers) {
        ...
        const allocatedConsumers = consumers.filter(c => c.status === enums_1.ConsumerStatus.ALLOCATED && c.allocatedPercentage);
        let allocatedKwh = 0;
    ...
    groupConsumersByStatus(consumers) {
        const statusGroups = {
            allocated: consumers.filter(c => c.status === enums_1.ConsumerStatus.ALLOCATED),
            inProcess: consumers.filter(c => c.status === enums_1.ConsumerStatus.IN_PROCESS),
            converted: consumers.filter(c => c.status === enums_1.ConsumerStatus.CONVERTED),
            available: consumers.filter(c => c.status === enums_1.ConsumerStatus.AVAILABLE),
        };
        return {
            allocated: {
    ...
    calculateGeographicDistribution(consumers) {
        ...
        consumers.forEach(consumer => {
            ...
            if (consumer.status === enums_1.ConsumerStatus.ALLOCATED && consumer.allocatedPercentage) {
