...
describe("ConsumersController", () => {
  ...
  describe("create", () => {
    it("should successfully create a consumer", async () => {
      ...
      expect(mockConsumersService.create).toHaveBeenCalledWith(
        createConsumerDto,
      );
    });
    ...
    it("should handle creation errors", async () => {
      const createConsumerDto = createValidConsumerDto();
      mockConsumersService.create.mockRejectedValue(
        new Error("Creation failed"),
      );

      await expect(controller.create(createConsumerDto)).rejects.toThrow(
        "Creation failed",
      );
    });
  });
  ...
  describe("update", () => {
    it("should successfully update a consumer", async () => {
      ...
      expect(mockConsumersService.update).toHaveBeenCalledWith(
        consumerId,
        updateConsumerDto,
      );
    });
  ...
  describe("findPending", () => {
    it("should return pending consumers with pagination", async () => {
      const paginatedResult = {
        consumers: [
          { ...mockConsumer, approvalStatus: ConsumerApprovalStatus.PENDING },
        ],
        pagination: {
          page: 1,
          limit: 20,
  ...
  describe("approveConsumer", () => {
    it("should approve a pending consumer", async () => {
      ...
      expect(mockConsumersService.approveConsumer).toHaveBeenCalledWith(
        consumerId,
        userId,
      );
    });
  ...
  describe("rejectConsumer", () => {
    it("should reject a pending consumer", async () => {
      ...
      const rejectedConsumer = {
        ...mockConsumer,
        approvalStatus: ConsumerApprovalStatus.REJECTED,
      };
      ...
      expect(mockConsumersService.rejectConsumer).toHaveBeenCalledWith(
        consumerId,
        userId,
        reason,
      );
  ...
  describe("findRepresentativeConsumer", () => {
    it("should find consumer by representative", async () => {
      ...
      mockConsumersService.findRepresentativeConsumer.mockResolvedValue(
        representativeConsumer,
      );

      // Call service directly to avoid Request object issues
      const result = await service.findRepresentativeConsumer(
        representativeId,
        consumerId,
      );
      ...
      expect(
        mockConsumersService.findRepresentativeConsumer,
      ).toHaveBeenCalledWith(representativeId, consumerId);
    });

    it("should throw error if representative consumer not found", async () => {
      mockConsumersService.findRepresentativeConsumer.mockRejectedValue(
        new Error("Not found"),
      );
  ...
  describe("createAsRepresentative", () => {
    it("should create consumer as representative", async () => {
      ...
      mockConsumersService.createAsRepresentative.mockResolvedValue(
        pendingConsumer,
      );
