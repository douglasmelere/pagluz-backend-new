...
describe("ConsumersService", () => {
  ...
  describe("create", () => {
    ...
    it("should create consumer without generator if not specified", async () => {
      ...
      mockPrismaService.consumer.create.mockResolvedValue(
        consumerWithoutGenerator,
      );
  ...
  describe("createAsRepresentative", () => {
    it("should successfully create consumer as representative with pending approval", async () => {
      ...
      const result = await service.createAsRepresentative(
        createConsumerDto,
        representativeId,
      );
    ...
    it("should throw error if representative not found", async () => {
      ...
      await expect(
        service.createAsRepresentative(createConsumerDto, representativeId),
      ).rejects.toThrow(new NotFoundException("Representante não encontrado"));
    });
    ...
    it("should throw error if generator not found when specified by representative", async () => {
      ...
      await expect(
        service.createAsRepresentative(createConsumerDto, representativeId),
      ).rejects.toThrow(new NotFoundException("Gerador não encontrado"));
    });
  });
  ...
  describe("update", () => {
    ...
    it("should throw error if consumer not found during update", async () => {
      ...
      await expect(
        service.update(consumerId, updateConsumerDto),
  ...
  describe("remove", () => {
    ...
    it("should throw error if consumer not found during deletion", async () => {
      ...
      await expect(service.remove(consumerId)).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe("linkToRepresentative", () => {
    ...
    it("should find specific representative consumer", async () => {
      ...
      mockPrismaService.consumer.findFirst.mockResolvedValue(
  ...
  describe("rejectConsumer", () => {
    ...
    it("should throw error if consumer not found", async () => {
      ...
      await expect(
        service.rejectConsumer("1", "user-1", "reason"),
      ).rejects.toThrow(NotFoundException);
    });
  });
