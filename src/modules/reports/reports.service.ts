...
// Importação dinâmica do ExcelJS (CommonJS)
...
export class ReportsService {
  ...
  async generateCommissionsReport(res: Response, filters: {
  ...
  // ─── Relatório de Consumidores ──────────────────────────────────────────────

  async generateConsumersReport(res: Response, filters: {
    representativeId?: string;
    status?: string;
    ...
    if (filters.representativeId) where.representativeId = filters.representativeId;
    if (filters.status) where.status = filters.status;
    if (filters.concessionaire) where.concessionaire = filters.concessionaire;
  ...
  async generateRepresentativesReport(res: Response) {
