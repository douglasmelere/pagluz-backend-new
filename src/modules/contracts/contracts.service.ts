...
export class ContractsService {
  ...
  private readonly TEMPLATE_LOCACAO_ID =
    "1BjjCJGisw9baDI1ENQpiykgh7ZBUY5vPbcwuNzOU9N4";
  private readonly TEMPLATE_PRESTACAO_ID =
    "1qZxkafpOE4BFuRrZqdraqpAcXwUE_7GsTY0Q5bzcmdY";
  private readonly TEMPLATE_PROCURACAO_PJ_ID =
    "1UtYkaU0Y8bq-_3Sm7V-Jsz_8leUOMDQtpRG2idcxUWk";
  private readonly TEMPLATE_PROCURACAO_PF_ID =
    "1qtI83buiWR7TKuxNQdEZf33vsli2mGNIWgmCoGcuC_A";
  ...
  private readonly SPREADSHEET_ID =
    "19DtKJnOMxw4TVbc8sbywV5oP6x6wzps7iFuuAmLshTg";
  ...
  private async generateLocacaoContract(
    ...
    try {
      ...
      const folderId = await this.googleApis.createFolder(
        folderName,
        this.FOLDER_LOCACAO_ID,
      );
  ...
  private async generatePrestacaoContract(
    ...
    try {
      ...
      // Adiciona linha no Google Sheets
      await this.googleApis.appendRow(
        this.SPREADSHEET_ID,
        this.SHEET_PRESTACAO,
        {
          "ID CONTRATO": contractId,
          ...
          "CPF DO REPRESENTANTE DO CONTRATANTE":
            data.cpfRepresentanteContratante,
          CIDADE: data.cidade,
          DATA: data.data,
        },
      );
      ...
      if (!existingGenerator) {
        ...
        await this.prisma.generator.create({
          data: {
            ...
            sourceType: this.mapTipoUsinaToSourceType(
              data.tipoUsina,
            ) as SourceType,
  ...
  private mapTipoUsinaToSourceType(tipoUsina: string): SourceType {
    const mapping: { [key: string]: SourceType } = {
      solar: SourceType.SOLAR,

      hidro: SourceType.HYDRO,

      eolica: SourceType.WIND,

      biomassa: SourceType.BIOMASS,
    };

    return mapping[tipoUsina?.toLowerCase()] || SourceType.SOLAR;
  }
}
